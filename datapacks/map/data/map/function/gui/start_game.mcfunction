...
execute if score #developer_tools map.global matches 0 if entity @a[team=map.red] unless entity @a[team=map.blue] run tellraw @a [{"text":"Cannot start game as there are no players on "},{"text":"Blue Team","color":"blue"}]
execute if score #developer_tools map.global matches 0 unless entity @a[team=map.red] if entity @a[team=map.blue] run tellraw @a [{"text":"Cannot start game as there are no players on "},{"text":"Red Team","color":"red"}]
execute unless entity @a[team=map.red] unless entity @a[team=map.blue] run tellraw @a [{"text":"Cannot start game as there are no players on either "},{"text":"Red Team ","color":"red"},{"text":"or "},{"text":"Blue Team","color":"blue"}]
