...
execute unless data storage map:data {hp:"0.0"} on attacker run tellraw @s[distance=8..] [{text:""},{selector:"@a[tag=map.selected]"},{text:" is on ",color:"gray"},{storage:"map:data",nbt:"hp",interpret:true,color:"gold"},{text:" HP",color:"gray"}]
