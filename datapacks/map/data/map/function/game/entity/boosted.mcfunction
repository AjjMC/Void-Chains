...
execute if entity @s[tag=map.anchor_powerup,team=map.red] run title @s actionbar [{text:"Red Team ",color:"red"},{text:"Bridges You Stand Above Are Unaffected by Gravity!",color:"green"}]
execute if entity @s[tag=map.anchor_powerup,team=map.blue] run title @s actionbar [{text:"Blue Team ",color:"blue"},{text:"Bridges You Stand Above Are Unaffected by Gravity!",color:"green"}]
