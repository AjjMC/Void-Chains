...
execute unless entity @e[type=minecraft:marker,tag=map.red_anchor] if entity @a[team=map.blue] unless entity @a[team=map.red,tag=!map.dead] run function map:events/blue_team_win
execute unless entity @e[type=minecraft:marker,tag=map.blue_anchor] if entity @a[team=map.red] unless entity @a[team=map.blue,tag=!map.dead] run function map:events/red_team_win

execute unless entity @a[team=!map.guest] if entity @a[team=map.guest] run function map:events/draw

execute if score #developer_tools map.global matches 0 if entity @e[type=minecraft:marker,tag=map.red_anchor] if entity @a[team=map.blue] unless entity @a[team=map.red] run function map:events/blue_team_win
execute if score #developer_tools map.global matches 0 if entity @e[type=minecraft:marker,tag=map.blue_anchor] if entity @a[team=map.red] unless entity @a[team=map.blue] run function map:events/red_team_win