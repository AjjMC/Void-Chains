execute if score #powerup_seconds map.global = #powerup_timeout_seconds map.global as @e[type=minecraft:item,tag=map.powerup] at @s run function map:game/arena/powerup/destroy
execute if score #powerup_generation map.global matches 1 if score #powerup_minutes map.global = #powerup_generation_minutes map.global at @e[type=minecraft:marker,tag=map.arena] run function map:game/arena/powerup/generate
