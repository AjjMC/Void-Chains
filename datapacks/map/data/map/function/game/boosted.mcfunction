...
execute if entity @s[tag=map.anchor_powerup,team=map.red] run title @s actionbar [{"text":"Red Territory ","color":"red"},{"text":"You Stand on Is Unaffected by Gravity!","color":"green"}]
execute if entity @s[tag=map.anchor_powerup,team=map.blue] run title @s actionbar [{"text":"Blue Territory ","color":"blue"},{"text":"You Stand on Is Unaffected by Gravity!","color":"green"}]
